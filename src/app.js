...
function addAdvancedUI() {
  ...
  const group = document.createElement('select'); group.id = 'group-by'; group.title = 'Kelompokkan library'; group.ariaLabel = 'Kelompokkan library'; group.innerHTML = '<option value="">Semua lagu</option><option value="artist">Artis</option><option value="album">Album</option><option value="genre">Genre</option>'; $('.table-actions').insertBefore(group, $('#sort'));
  const waveform = document.createElement('canvas'); waveform.id = 'waveform'; waveform.setAttribute('aria-label', 'Waveform audio'); $('.spectrum-box').append(waveform);
  const dialog = document.createElement('dialog'); dialog.id = 'settings-dialog'; dialog.innerHTML = `<form method="dialog" class="dialog-heading"><div><div class="eyebrow">ATIGA AMP / CONTROL ROOM</div><h2>Pengaturan<span>.</span></h2></div><button class="icon-button" aria-label="Tutup">×</button></form><section class="theme-section" aria-labelledby="theme-heading"><div class="settings-section-heading"><span id="theme-heading">Pilih skin</span><small>Perubahan langsung diterapkan</small></div><div class="theme-picker" role="radiogroup" aria-label="Pilih tema"><button type="button" class="theme-skin skin-ember" data-theme="ember" role="radio" aria-label="Ember"><span class="skin-preview"><i></i><i></i><i></i></span><strong>Ember</strong><small>Warm studio</small></button><button type="button" class="theme-skin skin-neon" data-theme="neon" role="radio" aria-label="Neon"><span class="skin-preview"><i></i><i></i><i></i></span><strong>Neon</strong><small>Night club</small></button><button type="button" class="theme-skin skin-ocean" data-theme="ocean" role="radio" aria-label="Ocean"><span class="skin-preview"><i></i><i></i><i></i></span><strong>Ocean</strong><small>Deep blue</small></button><button type="button" class="theme-skin skin-violet" data-theme="violet" role="radio" aria-label="Violet"><span class="skin-preview"><i></i><i></i><i></i></span><strong>Violet</strong><small>Midnight pulse</small></button><button type="button" class="theme-skin skin-mono" data-theme="mono" role="radio" aria-label="Monochrome"><span class="skin-preview"><i></i><i></i><i></i></span><strong>Mono</strong><small>Pure contrast</small></button></div></section><div class="settings-grid"><label>Crossfade <output id="crossfade-value"></output><input id="crossfade" type="range" min="0" max="12" step="1"></label><label>Preamp <output id="preamp-value"></output><input id="preamp" type="range" min="-12" max="12" step="1"></label><label>Balance <output id="balance-value"></output><input id="balance" type="range" min="-1" max="1" step=".01"></label><label>Output<select id="output-device"><option value="default">Default browser output</option></select></label></div><label class="setting-check"><input id="gapless" type="checkbox"> Gapless / preload lagu berikutnya</label><label class="setting-check"><input id="replay-gain" type="checkbox"> ReplayGain jika tag tersedia</label><label class="setting-check"><input id="glow" type="checkbox"> CRT / glow effect</label><label class="setting-check"><input id="notifications" type="checkbox"> Notifikasi lagu berikutnya</label><div class="settings-actions"><button type="button" class="text-button" id="scan-folder">Pindai ulang folder</button><button type="button" class="text-button" id="request-notifications">Izinkan notifikasi</button><span class="dialog-note">Panel dapat dipindahkan dengan drag-and-drop.</span></div><div class="format-support" id="format-support"></div>`; document.body.append(dialog);
...
function syncEQ() { $$('#eq-bands input').forEach((input,i) => { input.value = state.eq[i]; $(`#gain-${i}`).textContent = `${state.eq[i]>0?'+':''}${state.eq[i]}`; if (filters[i]) filters[i].gain.setTargetAtTime(state.eq[i], context.currentTime,.03); }); $('#eq-preset').value = state.preset; $('#preset-label').textContent = state.preset; persist(); }
...
async function init() {
  ...
  render(); $('#eq-preset').value = state.preset; $('#preset-label').textContent = state.preset;
