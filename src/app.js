...
function addAdvancedUI() {
  ...
  dialog.querySelector('#theme').onchange = event => { state.theme = event.target.value; applyTheme(); persist(); };
...
function syncSettings() { const set = (id, value) => { const el = $(`#${id}`); if (!el) return; if (el.type === 'checkbox') el.checked = value; else el.value = value; }; set('theme', state.theme); set('crossfade', state.crossfade); set('preamp', state.preamp); set('balance', state.balance); set('gapless', state.gapless); set('replay-gain', state.replayGain); set('glow', state.glow); set('notifications', state.notifications); set('output-device', state.outputDevice); $('#crossfade-value').textContent = `${state.crossfade}s`; $('#preamp-value').textContent = `${state.preamp > 0 ? '+' : ''}${state.preamp} dB`; $('#balance-value').textContent = state.balance === 0 ? 'Center' : state.balance < 0 ? `${Math.round(-state.balance * 100)}% L` : `${Math.round(state.balance * 100)}% R`; }
